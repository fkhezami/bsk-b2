...
export function PublicHeader() {
  ...
  return (
    <header style={{ background: "#0f0e2e" }} className="border-b border-brand-900 sticky top-0 z-50">
      <div className="max-w-4xl mx-auto px-4 py-3 flex items-center justify-between gap-6">
        ...
        <nav className="flex items-center gap-1">
          {navLinks.map(({ href, label, icon: Icon }) => {
            ...
            return (
              ...
              >
                <Icon className={cn("w-4 h-4", active ? "text-gold-400" : "text-brand-400")} />
                {label}
              </Link>
            );
          })}
          ...
          <Link
            href="/admin/queue"
            className={cn(
              "flex items-center gap-2 px-3 py-2 rounded-lg text-sm font-medium transition-all ml-2 border border-brand-700",
              pathname.startsWith("/admin")
                ? "bg-brand-700/80 text-white border-brand-500"
                : "text-brand-400 hover:text-white hover:bg-white/5"
            )}
          >
            <LayoutGrid className={cn("w-4 h-4", pathname.startsWith("/admin") ? "text-gold-400" : "text-brand-500")} />
            Admin
          </Link>
        </nav>
      </div>
